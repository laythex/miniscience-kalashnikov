...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            xc += nodesCoords[i*3];
            yc += nodesCoords[i*3 + 1];
            zc += nodesCoords[i*3 + 2];
        }
        xc /= (nodesCoords.size() / 3);
        yc /= (nodesCoords.size() / 3);
        zc /= (nodesCoords.size() / 3);
        std::cout << "Calculated center of mass - X: " << xc << "; Y: " << yc << "; Z: " << zc << std::endl;
        xe = 0;
        ye = 0;
        unsigned ne = 0;
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            // Координаты заберём из gmsh
            double pointX = nodesCoords[i*3] - xc;
            double pointY = nodesCoords[i*3 + 1] - yc;
            double pointZ = nodesCoords[i*3 + 2] - zc;
            if (pointZ > 50) {
                xe += pointX;
                ye += pointY;
                ne++;
            ...
            nodes[i] = CalcNode(pointX, pointY, pointZ, smth, 0, 0, 0);
        }

        xe /= ne;
        ye /= ne;
        std::cout << xe << " " << ye << std::endl;
    ...
    void updateVelocity() {
        for (unsigned i = 0; i < nodes.size(); i++) {
            if (nodes[i].z < 50) continue;
            double factor = (nodes[i].z / 50 - 1) * 10;
            nodes[i].vx = -(nodes[i].y - ye) * factor;
            nodes[i].vy = (nodes[i].x - xe) * factor;
            nodes[i].vz = factor * 3;
        }
    }
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "bunny-step-" + std::to_string(snap_number) + ".vtu";
...
int main(int argc, char **argv)
{
    ...
    for(unsigned int step = 1; step < 200; step++) {
        mesh.doTimeStep(tau);
        mesh.snapshot(step);
